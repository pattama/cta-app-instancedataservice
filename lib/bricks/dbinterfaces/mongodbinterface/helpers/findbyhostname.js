 ...
 */
class FindByHostname extends BaseDBInterfaceHelper {
  ...
  _validate(context) { // eslint-disable-line no-unused-vars
    ...
    return new Promise((resolve, reject) => { // eslint-disable-line no-unused-vars
      ...
      if (!instance.payload.hasOwnProperty('hostname')
        || typeof instance.payload.id !== 'string'
        || !(ObjectID.isValid(instance.payload.hostname))) {
        reject(new Error('missing/incorrect \'hostname\' String value of ObjectID in job payload'));
      }
      resolve({ ok: 1 });
    });
  }
  ...
  _process(context) {
    ...
    output.on('done', function(brickname, response) {
      if (Array.isArray(response) && response.length > 0) {
        const result = _.cloneDeep(response[0]);
