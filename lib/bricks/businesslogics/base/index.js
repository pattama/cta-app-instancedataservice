...
class Base extends Brick {
  ...
  validate(context) {
    ...
    return co(function* validateCoroutine() {
      ...
      const type = instance.nature.type.trim().toLowerCase();
      if (type !== that.constructor.name.toLowerCase()) {
        throw (new Error(`type ${instance.nature.type} not supported`));
      }

      const quality = instance.nature.quality.trim().toLowerCase();
      if (!that.helpers.has(quality)) {
        throw (new Error(`quality ${instance.nature.quality} not supported`));
      }
