...
class Handler {
  ...
  constructor(cementHelper) {
    ...
    this.collection = null; // to be set by child class
  }

  _create(req, res, content) {
    const data = {
      nature: {
        type: 'document',
        quality: 'create',
    ...
    if (req.method.toLowerCase() === 'put' && !req.params.hasOwnProperty('id')) {
      ...
      if (req.params.hasOwnProperty('id')) {
        data.payload.content.id = req.params.id;
      }
  ...
  update(req, res) {
    ...
    context.on('done', function(brickname, response) {
      ...
      } else if (req.headers.create === 'yes'){
        // attempt creation
        that._create(req, res, _.assignIn(req.body, req.query));
      } else {
